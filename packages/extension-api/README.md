...
profile and financial values, and normalized partial failures for broad Snapshot acquisition. The
security-research capability carries one exact Snapshot security request, detailed price history,
retrieval-time company facts, statement facts, events, and normalized failures.

External packages should depend on `marketsieve-extension-api>=0.11,<0.12` and the matching SDK minor
