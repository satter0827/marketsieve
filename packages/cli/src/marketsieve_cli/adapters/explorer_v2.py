...
function line(series,unit){const all=series.flatMap(s=>s.rows.map(r=>num(r.value)).filter(v=>v!==null));if(all.length<2)return'';const W=720,H=260,p=42,min=Math.min(...all,0),max=Math.max(...all,0),y=v=>H-p-(v-min)/(max-min||1)*(H-2*p);return`<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="横軸 日付、縦軸 ${esc(unit)}"><line class="axis" x1="${p}" y1="${H-p}" x2="${W-p}" y2="${H-p}"/><line class="axis" x1="${p}" y1="${p}" x2="${p}" y2="${H-p}"/><text x="${W/2}" y="${H-7}" text-anchor="middle" fill="currentColor">日付</text><text x="12" y="${H/2}" transform="rotate(-90 12 ${H/2})" text-anchor="middle" fill="currentColor">${esc(unit)}</text>${series.map((s,j)=>`<polyline class="series s${j%3}" points="${s.rows.map((r,i)=>`${p+i/(s.rows.length-1||1)*(W-2*p)},${y(num(r.value))}`).join(' ')}"><title>${esc(s.name)} · ${esc(unit)}</title></polyline>`).join('')}${series.map((s,j)=>`<text x="${p+j*150}" y="18" fill="currentColor">${j+1}: ${esc(s.name)}</text>`).join('')}</svg>`}
function candles(rows,unit){if(rows.length<2)return'';const W=720,H=260,p=42,values=rows.flatMap(r=>[num(r.low),num(r.high)]),lo=Math.min(...values),hi=Math.max(...values),y=v=>H-p-(v-lo)/(hi-lo||1)*(H-2*p),step=(W-2*p)/rows.length,w=Math.max(1,Math.min(8,step*.65));return`<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="横軸 日付、縦軸 ${esc(unit)} のローソク足"><line class="axis" x1="${p}" y1="${H-p}" x2="${W-p}" y2="${H-p}"/>${rows.map((r,i)=>{const x=p+(i+.5)*step,o=num(r.open),c=num(r.close),h=num(r.high),l=num(r.low),color=c>=o?'var(--accent)':'var(--neg)';return`<g><line x1="${x}" y1="${y(h)}" x2="${x}" y2="${y(l)}" stroke="${color}"/><rect x="${x-w/2}" y="${Math.min(y(o),y(c))}" width="${w}" height="${Math.max(1,Math.abs(y(o)-y(c)))}" fill="${color}"><title>${esc(r.date)} O ${o} H ${h} L ${l} C ${c}</title></rect></g>`}).join('')}<text x="${W/2}" y="${H-7}" text-anchor="middle" fill="currentColor">日付</text></svg>`}
