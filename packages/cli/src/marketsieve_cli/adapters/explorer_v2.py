...
function valueCell(r,f){if(r.values?.[f]!=null)return format(r.values[f],fieldMap.get(f)?.unit);return`<span class="meta">${esc(r.missing?.[f]||'未取得')}</span>`}
