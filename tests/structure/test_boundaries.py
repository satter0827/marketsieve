...
def test_ai_is_independent_from_cli_sources_and_io() -> None:
    ai_source = ROOT / "packages" / "ai" / "src" / "marketsieve_ai"
    imports = set().union(*(imported_roots(path) for path in ai_source.rglob("*.py")))
