...
VS Codeの「実行とデバッグ」を開くと、実行順を示す番号付き構成が先頭に並びます。最初の1回だけ`01`から`03`を順番に実行します。その後は各市場の終値後に`10`または`20`、週末にだけ`30`を実行します。生成された依頼ファイルをChatGPTへ渡して回答JSONを保存したら、`40`で取込と表示まで完了します。同名のTasksも代替入口として利用できます。
